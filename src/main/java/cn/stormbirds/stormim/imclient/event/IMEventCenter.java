package cn.stormbirds.stormim.imclient.event;
...
public class IMEventCenter {
    ...
    public static void dispatchEvent(CEvent event) {
        ...
        if(null != event && !StringUtil.isNullOrEmpty(event.topic)) {
            ...
            synchronized (mListenerLock) {
                log.error(TAG, "dispatchEvent | topic = " + topic + " msgCode = " + event.msgCode);
                Object obj = mListenerMap.get(topic);
                if(null != obj) {
