package cn.stormbirds.stormim.imclient.netty;
...
public class NettyTcpClient implements IMClientInterface {
    ...
    private String serverUrls;// ims服务器地址组
     ...
     * 初始化
     *
     * @param serverUrls 服务器地址列表
     * @param listener      与应用层交互的listener
     * @param callback      ims连接状态回调
     */
    @Override
    public void init(String serverUrls, OnEventListener listener, IMSConnectStatusCallback callback) {
        close();
        isClosed = false;
    ...
    private class ResetConnectRunnable implements Runnable {
        ...
        private int connectServer() {
            ...
            for (int i = 0; (!isClosed && i < serverUrls.split(",").length); i++) {
                String serverUrl = serverUrls.split(",")[i];
                // 如果服务器地址无效，直接回调连接状态，不再进行连接
